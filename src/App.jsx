...
function App () {
  ...
  trapsJson.trap.map(trap =>
    traps.push(new Trap(trap.id, trap.description, trap.damage, trap.weight))
  )

  treasuresJson.treasure.map(treasure =>
    treasures.push(
      new Treasure(
        treasure.id,
        treasure.name,
        ...
        treasure.weight,
        treasure.inventory.map(el => getInventory(el))
      )
    )
  )

  monstersJson.monster.map(monster =>
    monsters.push(
      new Monster(
        monster.id,
        monster.name,
        ...
        monster.weight,
        monster.description,
        [...monster.inventory.map(el => getInventory(el))]
      )
    )
  )

  playersJson.player.map(player =>
    players.push(
      new Player(
        player.id,
        player.type,
        ...
        player.currentLP,
        player.weight,
        [...player.inventory.map(el => getInventory(el))]
      )
    )
  )

  roomsJson.room.map(room =>
    rooms.push(
      new Room(room.id, room.name, room.description, [
        ...room.objects.map(el => getInventory(el))
      ])
    )
  )
  ...
  return (
    <div className='App'>
      <Controllers
        ...
        player={players[0]}
        nextRoom={rooms[roomIndex + 1]}
        setRoomIndex={incrementRoomIndex}
        endGame={() => setRoomIndex(9)}
      ></Controllers>
    </div>
  )
}
